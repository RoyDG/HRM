...
if (isset($_POST['submit']) && isset($_POST['report']) && $_POST['report'] > 0) {
	...
	if (isset($dealer_type)) {
		$dtype_con = ' and d.dealer_type="' . $dealer_type . '"';
	}
	...
	if (isset($depot_id)) {
		$depot_con = ' and m.depot_id="' . $depot_id . '"';
	}
	...
	<div class="main">
		...
		if ($_POST['report'] == 1004) {
			...
			if (isset($sub_group_id)) {
				$item_sub_con = ' and i.sub_group_id=' . $sub_group_id;
			...
			if (isset($product_group)) {
				$product_group_con = ' and i.sales_item_type="' . $product_group . '"';
			}
		...
		if ($_POST['report'] == 100245) {
			...
			if (isset($_POST['sub_group_id'])) {
				$item_sub_con .= ' and i.sub_group_id=' . $_POST['sub_group_id'];
			}
...
c.user_id=a.entry_by and a.item_id=i.item_id' . $con . $date_con . $warehouse_con . $item_con . $item_brand_con . $dtype_con . ' group by  d.dealer_name_e order by d.dealer_name_e  ';
		...
		?>
			...
			<!-- Start Of DO-Tracking report -->


		<? } elseif ($_POST['report'] == 606) {
			...
			if (isset($t_date)) {
				$to_date = $t_date;
				$fr_date = $f_date;
				$date_con = ' and a.jv_date between \'' . $fr_date . '\' and \'' . $to_date . '\'';
			}
			...
			$sql66 = "select d.dealer_name_e as dealer_name, j.jv_no, j.jv_date, j.cr_amt as amount
			 from journal j inner join dealer_info d on d.account_code= j.ledger_id
			  where ref_no = 0 and from_unixtime(jv_date, '%Y-%m-%d')
			between '" . $fr_date . "' and '" . $to_date . "'and tr_from='receipt'";
		...
		?>
			...
			<table width="100%" cellspacing="0" cellpadding="2" border="0">
				...
				<thead>
					<tr>
						<td style="border:0px;" colspan="3"><?= $str ?></td>
					</tr>
					<tr>
						<th>S/L</th>
						<th>Dealer Name</th>
				...
				<tbody>
					...
					?>
						...
						<tr>
							...
							<td>
								<a target="_blank" href="../../../acc_mod/pages/voucher_view_do.php?v_no=<?= $data->jv_no ?>">
									<?= $data->jv_no?>
								</a>
			...
			<!-- End Of DO-Tracking report (Rahul) -->
		...
		<?
			...
			if (isset($marketing_person)) {
				$con .= ' and d.marketing_person="' . $marketing_person . '"';
			}
	...
	$days_106= $days_105-86400;*/
					...
					?>
							...
							<tr>
								...
								<td align="right"><?= number_format($closing);
													$total_closing_amount = $total_closing_amount + $closing; ?></td>
...
where ledger_id = "' . $data->account_code . '" and tr_from="Sales" and jv_date between "' . $days_30 . '" and "' . $today_days . '" group by tr_id ';
													...
													$check_dues_30 = ($sales_30 - ($rceived_30 + $return_30 + $expense_30));
													$total_30_days_dues += $check_dues_30;
...
where ledger_id = "' . $data->account_code . '" and tr_from="Sales" and jv_date between "' . $days_60 . '" and "' . $days_31 . '" group by tr_id ';
													...
													$total_60_days_dues += $check_dues_60;
...
where ledger_id = "' . $data->account_code . '" and tr_from="Sales" and jv_date between "' . $days_90 . '" and "' . $days_61 . '" group by tr_id ';
													...
													$total_90_days_dues += $check_dues_90;
...
where ledger_id = "' . $data->account_code . '" and tr_from="Sales" and jv_date between "' . $days_120 . '" and "' . $days_91 . '" group by tr_id ';
													...
													$check_dues_120 = ($sales_120 - ($rceived_120 + $return_120 + $expense_120));
													$total_120_days_dues += $check_dues_120;
													...
													//$day75=find_a_field('journal','sum(dr_amt-cr_amt)','ledger_id="'.$data->account_code.'" and jv_date between "'.$days_75.'" and "'.$days_60.'"');
													?></td>
...
where ledger_id = "' . $data->account_code . '" and tr_from="Sales" and jv_date between "' . $aging_fixed_back_date . '" and "' . $days_121 . '" group by tr_id ';
									...
									$check_dues_avobe_120 = ($sales_avobe_120 - ($rceived_avobe_120 + $return_avobe_120 + $expense_avobe_120));
									$total_dues_avobe_120 += $check_dues_avobe_120;
...
<tbody>';
			...
			?>
				...
				<tr>
					...
					<td align="right"><? if ($data->vat_cal_type == 'inclusive' && $data->unit_price > 0) {
											echo $net_ret;
										} else {
											echo $data->unit_price;
										} ?></td>
					...
					<td align="right"><?= ($ait_amount > 0) ? number_format($ait_amount) : '';
										$total_ait_amount += $ait_amount; ?></td>

					<td align="right"><? if ($data->vat == 0 && $data->vat != '') {
											echo " Vat Exempted ";
										} elseif ($data->vat > 0) {
											echo $data->vat;
										} ?></td>

					<td align="right"><? if ($data->vat != 0 && $data->unit_price > 0) {
											echo number_format($vat_amount);
...
d.team_name as team,w.warehouse_name as depot,d.product_group as grp,c.driver_name,d.commission,m.cost_center
...
<thead><tr><td style="border:0px;" colspan="7">' . $str . '</td></tr><tr><th>S/L</th><th>Chalan No</th><th>Do No</th><th>Do Date</th><th>Chalan Date</th>



<th>Dealer Name</th><th>Cost Center</th>
...
<tbody>';
			...
			?>
				...
				<tr>
					...
					<td align="right"><?= number_format($total_sales_amount);
										$gross_sales += $total_sales_amount; ?></td>



					<td align="right"><?= number_format($data->cogs_amount, 2);
										$total_cosg_amount += $data->cogs_amount; ?></td>



					<td align="right"><?= number_format($differentiation_total, 2);
										$total_differentiation += $differentiation_total; ?></td>
				</tr>
			...
			if (isset($t_date)) {
				$to_date = $t_date;
				$fr_date = $f_date;
				$or_date_con = ' and a.or_date between \'' . $fr_date . '\' and \'' . $to_date . '\'';
			}
...
<tbody>';
				...
				?>
					...
					<tr>
						...
						<td align="right"><?= number_format($differentiation_total, 2); ?></td>
					</tr>
...
where  m.status in ('CHECKED','COMPLETED') and m.dealer_code=d.dealer_code and w.warehouse_id=m.depot_id and a.AREA_CODE=d.area_code and s.do_no=m.do_no and a.ZONE_ID=" . $zone->ZONE_CODE . " " . $date_con . $pg_con . $area_con;
								...
								if ($t->total > 0) {
									...
									if ($rp == 0) {
										$reg_total = 0;
										$dp_total = 0;
										$str .= '<p style="width:100%">Region Name: ' . $branch->BRANCH_NAME . ' Region</p>';
										$rp++;
									}

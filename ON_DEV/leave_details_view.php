...
?>
 ...
 <tr style="font-weight:bold;">
   ...
   <?
       $basic = 'select p.PBI_ID, p.PBI_NAME,p.PBI_CODE,
      ...
      from personnel_basic_info p where p.PBI_ID="'.$PBI_ID.'"';
  ...
  <tr style="font-weight:bold; padding:10px;">
    ...
    <td style="">
    <span>
      <?=$basic_data->PBI_NAME?> [<?=$basic_data->PBI_CODE?>]
    </span>
  </td>
	<td>&nbsp;</td>
	<td>&nbsp;</td>
...
<br />
  ...
  <tr>
   ...
   <? }else{?>	
    <td>
      <?php echo find_a_field('hrm_leave_type','leave_type_name','id='.$data->type) ?>
    </td>
    <td>
      <?php echo date('d-M-Y',strtotime($data->s_date)) ?>
    </td>
    <td>
      <?php echo date('d-M-Y',strtotime($data->e_date)) ?>
    </td>
	<? } ?>

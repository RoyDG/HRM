...
$office_time = find_a_field('hrm_schedule_info','office_start_time_with_grace','schedule_name="General"');
$office_start_time = strtotime($today.' '.$office_time);

$lateSql = 'select min(h.xtime) as xtime from hrm_attdump h, personnel_basic_info p where p.PBI_ID=h.EMP_CODE and h.xdate="'.$today.'" and h.xenrollid>0 group by h.EMP_CODE';
$qrry = mysql_query($lateSql);
while($lt_data=mysql_fetch_object($qrry)){
 if(strtotime($lt_data->xtime)>$office_start_time){
 $late++;
